...
class Button(gtk.Button):
    ...
    def __init__(self, archivo, string=None, size=24, funcion=None):
...
class Login(gtk.Dialog):
    ...
    def __init__(self, http):
        ...
        self.usuario.connect('activate', lambda w: self.set_focus(self.clave))
        self.clave = Entry()
    ...
    def comprobar(self, *args):
        ...
        self.sessionid = self.http.login(self.user, self.cl)
        print 'LOGIN SEND', self.sessionid
        if self.sessionid:
            d = {self.mac: (self.user, self.cl)}
            js = json.dumps(d)
...
class TextView(gtk.TextView):
    ...
    def get_text(self):
        ...
        texto = buff.get_text(start, end, True)
        return texto
